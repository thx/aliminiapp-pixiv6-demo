...
const { Animate } = PIXI
import bombAnimate from "./bombscene";
Page({
  // 供pixi渲染的canvas
  canvas: null,
  context: null,
  ...
  onAppInit(e) {
    ...
    // 注意：如果导出的动画js不是自运行模式，需要提前使用 setup() 方法进行装载
    bombAnimate.setup(Animate);
    
    Animate.load(bombAnimate, {
      parent: application.stage,
      createInstance: true,
      complete: (e) => {
        console.log('pixi-animate loaded', e);
      }
    });
  ...
  onShareAppMessage() {
    // 返回自定义分享信息
    return {
      title: 'Demo-PIXIAnimate',
      desc: 'Demo-PIXIAnimate',
      path: 'pages/animate_pixi-animate/index',
    };
  },
});
